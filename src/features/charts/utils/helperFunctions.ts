...
export const calculateDateRange = (
  ...
  export const processDailyScores = (
    ...
    if (!assessments?.length) {
      return dateRange.map((_, index, arr) => ({
    ...
    return dateRange.map((date, index, arr) => {
      ...
      if (firstAssessmentIndex === -1 || index < firstAssessmentIndex) {
        return { score: CHART_CONSTANTS.DEFAULT_SCORE, dotType: 'filler' };
      }
    ...
    return dateRange.map((monday, index, arr) => {
      if (!assessments?.length) {
        return index === arr.length - 1
          ? { score: null, dotType: 'empty' as DotType }
          : { score: CHART_CONSTANTS.DEFAULT_SCORE, dotType: 'filler' as DotType };
      }
      ...
      const result =  weekAssessments.length === 1
        ? { score: weekAssessments[0].score, dotType: 'actual' as DotType }
        : {
            score: Math.round(
